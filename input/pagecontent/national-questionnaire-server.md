...
2. **Licence enforcement at the source.** Tier-B/C instruments (metadata-only, licensed) can be served access-controlled: metadata public, item texts behind an authorisation the licence holder grants — enforced once, centrally, instead of hoped-for at every site. The WHODAS and BDI-II situations are the concrete drivers.
3. **A home for the scoring service.** Channel B (`Library/$evaluate`) wants to live where the Libraries live; co-locating turns the server from a file store into the **authoritative scoring endpoint**.
