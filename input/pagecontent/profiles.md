<!-- TODO:REVIEW — migrated from the Simplifier tree during the template
     migration (page-map branch 4). Source:
     implementation-guides/MII-PRO-v2026-DE/MIIIGModulPRO/Technische-Implementierung/FHIR-Profile/Abstrakte-Profile/Index.page.md -->
...
| Profile | Resource type | Base | Description |
|--------|--------------|-------|-------------|
| [MII_PR_PRO_Questionnaire](StructureDefinition-mii-pr-pro-questionnaire.html) | Questionnaire | SDC Questionnaire | Basis for all PRO questionnaires |
| [MII_PR_PRO_QuestionnaireResponse](StructureDefinition-mii-pr-pro-questionnaire-response.html) | QuestionnaireResponse | SDC QuestionnaireResponse | Structure of completed questionnaires |
| [MII_PR_PRO_Score_Blueprint](StructureDefinition-mii-pr-pro-score-blueprint.html) | ObservationDefinition | FHIR R4 | Templates for PRO score definitions |
| [MII_PR_PRO_Score_Instance](StructureDefinition-mii-pr-pro-score-instance.html) | Observation | FHIR R4 | Concrete score observations |

### Inheritance Hierarchy

The abstract profiles form the basis of a clear inheritance hierarchy:

![Abstract profiles inheritance hierarchy](Abstract_Profiles.png)
...
2. **Inheritance required**: every PRO instrument must define concrete profiles that inherit from the abstract ones
3. **Complete specification**: concrete profiles must give every abstract element a concrete value or constraint
4. **Validation**: validators check that instances reference only concrete profiles
...
2. Concrete instrument profiles remain unchanged (they already inherit from the abstract profiles)
3. Instances must reference concrete profiles, not the abstract ones
4. Validation should be updated to check the abstract constraint
