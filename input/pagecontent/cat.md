...
- **Canonical item identities.** The [ID systematics](id-systematics.html) give every item a stable, instrument-independent linkId (e.g. the PHQ-D block namespace shared by PHQ-9, PHQ-15, GAD-7). An item bank is exactly this: items with identities independent of any fixed questionnaire.
...
- **Server-side scoring channel.** The [three-channel scoring architecture](scoring.html) already routes authoritative calculation through the server (`Library/$evaluate`, CQL) — the same place a CAT engine would live.
...
- **Licensing.** CAT requires the full item bank with IRT parameters — a materially different licensing scope than a fixed short form (for PROMIS: usage request via CPCOR, see the [PROMIS licensing section](promis.html)).
- **Score provenance.** A CAT score derives from a variable item set; how the Observation documents which items and which parameter version produced it is part of the item-based-architecture work on the [roadmap](module-description.html).
