...
Für PRO-Instrumente zählt das doppelt: Die Belastung der Patientinnen und Patienten sinkt deutlich (entscheidend für wiederholte Messung in der Versorgungsroutine), und die Scores bleiben auf derselben T-Score-Metrik wie die festen Kurzformen — ein PROMIS-Depressions-CAT und die [PROMIS Depression SF 4a](promis-depression.html) messen dasselbe Konstrukt auf derselben Skala.

### Warum CAT inhärent serverseitig ist

CAT passt nicht in den clientseitigen Scoring-Kanal ([Scoring](scoring.html), Kanal A): Das nächste Item lässt sich nicht in einen statischen Questionnaire vorausberechnen, weil es von der laufenden Merkmalsschätzung abhängt. Itemauswahl und Scoring liegen deshalb auf einem Server (Kanal B), und der Formular-Client wird zur schlanken Schleife: Item anzeigen → Antwort senden → nächstes Item empfangen.
...
- **Kanonische Item-Identitäten.** Die [ID-Systematik](id-systematics.html) gibt jedem Item einen stabilen, instrumentenunabhängigen linkId (z. B. den PHQ-D-Block-Namespace, den PHQ-9, PHQ-15 und GAD-7 teilen). Eine Itembank ist genau das: Items mit Identität unabhängig von jedem festen Fragebogen.
...
- **Serverseitiger Scoring-Kanal.** Die [Drei-Kanal-Scoring-Architektur](scoring.html) führt die autoritative Berechnung bereits über den Server (`Library/$evaluate`, CQL) — denselben Ort, an dem eine CAT-Engine liegen würde.
...
- **Lizenzierung.** CAT braucht die vollständige Itembank mit IRT-Parametern — ein wesentlich anderer Lizenzumfang als eine feste Kurzform (für PROMIS: Nutzungsanfrage via CPCOR, siehe [PROMIS-Lizenzierung](promis.html)).
- **Score-Provenienz.** Ein CAT-Score entsteht aus einem variablen Itemset; wie die Observation dokumentiert, welche Items und welche Parameterversion ihn erzeugt haben, gehört zur Item-basierten-Architektur-Arbeit der [Roadmap](module-description.html).
