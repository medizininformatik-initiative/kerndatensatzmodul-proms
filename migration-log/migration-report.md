...
| F10 | CI moved onto the template toolchain | legacy `ig-publisher.yml` (docker `:latest`, archived to migration-log/) replaced by the template's pinned one; `validation.yml`, `sync-skills.yml`, 40 template scripts, `skills/`+lock, `docs/recipes` vendored; secret names adapted to the org's `CDS_DEV_CLIENT_*`; `.gitignore` `package/` anchored to `/package/` (it was swallowing `ig-template/package/`); M10 duplicate heading fixed; 13 provenance headers on the sanctioned wording; 10 ALLOW entries | commits `64b1…`–`894b…` |
| F11 | Foreign canonical unpinned | the PROMIS-29 bundle claimed `modul-person …/Patient|2026.7.0` — the house MetaProfile RuleSet stamps OUR version onto any canonical; sole foreign use, now claimed unversioned (pre-existing defect, caught by the new strict gate) | commit `894b3bcd5` |
...
- **D10 — strict Java gate (template validation.yml) still red.** The new reusable-workflow gate fails on (a) the pre-existing error classes (D9, dom-3, ext-1) and (b) the CRMI profile claims on the IG resource, which the reusable validator does not resolve even though `hl7.fhir.uv.crmi 2.0.0` is a declared dependency. Options: fix the pre-existing classes, or set `ENABLE_VALIDATION=false` until they are fixed, or investigate the reusable workflow's package loading. The legacy validation path (main.yml) passes. *If nobody acts:* the PR check stays red on every push.
